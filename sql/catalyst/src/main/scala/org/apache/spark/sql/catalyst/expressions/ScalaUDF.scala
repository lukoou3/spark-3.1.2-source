...
case class ScalaUDF(
  ...
  /**
   * f是个lazy的函数类型, 根据func和参数的个数生成对应的处理函数, 里面对调动createToScalaConverter转换sql类型成scala类型
   * 看这个注释, 这个函数定义的是有下面的脚本生成的, 可以的。自定义的udf, 参数个数支持0 - 22, 能够理解
   * scala的Function就定义Function0到Function22, Tuple也是Tuple1到Tuple22, Function和Tuple都是参数小于2的标记了@specialized, 要不然生成的原生类太多了
   *
   */
